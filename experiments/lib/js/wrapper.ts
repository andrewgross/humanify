/**
 * Detects the wrapper IIFE of a humanified bundle — where its top-level
 * statements live. Moved from the TS pipeline's
 * src/analysis/wrapper-detection.ts at the cutover
 * (docs/rust-port/19-cutover.md); the scorer's statement extraction
 * (experiments/lib/trees.ts `bundleStatements`) is its one caller.
 */
