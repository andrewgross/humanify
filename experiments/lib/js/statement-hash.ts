/**
 * Rename-invariant structural hash of wrapper-body statements — the eval
 * scorer's statement identity (`statementsOf`, every `novel` / `realLn` /
 * `noise` KPI keys on it).
 *
 * THE KPI HASH: moved byte-for-byte from the TS pipeline's
 * src/split/statement-hash.ts at the cutover (docs/rust-port/19-cutover.md,
 * proven by re-scoring three labels to byte-identical cards). The pipeline's
 * own split key is now the Rust binary's; this copy is owned by the harness
 * and must never change, or every reference on record stops being
 * comparable. It began life as the split's content-identity key for the
 * hash-keyed file inheritance:
 *
