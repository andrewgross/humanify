...
#
# Control leg = pass OFF (HUMANIFY_NO_NAME_ALIGN=1), candidate leg = pass ON.
...
if pgrep -f 'run\.sh exp050' >/dev/null 2>&1; then
...
echo "################ CONTROL LEG — hash-only keying (old), COLD ################"
env HUMANIFY_NO_NAME_ALIGN=1 REBASE_PRIOR=1 \
  experiments/034-eval-harness/run.sh exp050-cold-control "$WORK"
echo "CONTROL LEG EXIT=$?"

echo "################ CANDIDATE LEG — (hash,name) keying (new), COLD ################"
env REBASE_PRIOR=1 \
  experiments/034-eval-harness/run.sh exp050-cold "$WORK"
...
echo "################ VERDICT ################"
npx tsx experiments/034-eval-harness/leaderboard.ts exp050-cold-control exp050-cold
experiments/041-content-anchor/gate-verdict.sh exp050-cold-control exp050-cold
