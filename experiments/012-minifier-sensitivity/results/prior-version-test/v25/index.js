var FLAG_HYDRATE = 32;
var FLAG_SUSPENDED = 128;
var FLAG_INSERT_VNODE = 65536;
var FLAG_MATCHED = 1 << 17;
var RESET_FLAGS_MASK = ~(FLAG_HYDRATE | FLAG_SUSPENDED);
...
function mergeObjects(targetObj, sourceObj) {
  for (let nextState in sourceObj) targetObj[nextState] = sourceObj[nextState];
  return targetObj;
}
...
var defaultOptions = errorHandlingOptions;
...
var isValidElementInstance = (value) =>
...
function renderComponentInstance(componentInstance) {
  ...
  if (componentInstance._parentDom) {
    ...
    diffNodes(
      ...
      vnode._flags & FLAG_HYDRATE ? [domNode] : null,
      fiberArray,
      domNode == null ? findNextDomSibling(vnode) : domNode,
      !!(vnode._flags & FLAG_HYDRATE),
      refArray
    );
...
function propagateDomPointerToParent(parentComponent) {
  if (
    ...
    for (
      let childIndex = 0;
      childIndex < parentComponent._children.length;
      childIndex++
    ) {
      let childComponent = parentComponent._children[childIndex];
      if (childComponent != null && childComponent._dom != null) {
        parentComponent._dom = parentComponent._component.base =
          childComponent._dom;
        break;
      }
    }
...
function scheduleRender(component) {
  if (
    (!component._dirty &&
      ...
      rerenderQueueList.push(component) &&
      !processRerenderQueue._rerenderCount++) ||
    previousDebounceFn !== defaultOptions.debounceRendering
  ) {
    ((previousDebounceFn = defaultOptions.debounceRendering) || deferFn)(
...
function processRerenderQueue() {
  let component;
  for (
    rerenderQueueList.sort(sortByDepth);
    (component = rerenderQueueList.shift());

  ) {
    if (component._dirty) {
      let previousQueueLength = rerenderQueueList.length;
      renderComponentInstance(component);
      if (rerenderQueueList.length > previousQueueLength) {
        rerenderQueueList.sort(sortByDepth);
      }
    }
  }
...
function diffChildrenNodes(
  ...
  for (
    parentVNode._nextDom = nextSiblingNode,
      reconcileChildren(parentVNode, newChildren, oldChildren),
      nextSiblingNode = parentVNode._nextDom,
      childIndex = 0;
    ...
    if (currentChild != null) {
      ...
      if (
        currentChild._flags & FLAG_INSERT_VNODE ||
        ...
        nextSiblingNode = currentChild._nextDom;
      } else if (childDom) {
        nextSiblingNode = childDom.nextSibling;
      }
      currentChild._nextDom = UNDEFINED_VALUE;
      currentChild._flags &= ~(FLAG_INSERT_VNODE | FLAG_MATCHED);
    }
  }
  parentVNode._nextDom = nextSiblingNode;
  parentVNode._dom = firstDomNode;
}
function reconcileChildren(parentNode, newChildrenArray, oldChildrenArray) {
  let newChildIndex, currentChild, matchedSibling;
  const newChildrenLength = newChildrenArray.length;
  let oldChildrenLength = oldChildrenArray.length;
  let remainingOldChildrenCount = oldChildrenLength;
  let indexOffset = 0;
  for (
    parentNode._children = [], newChildIndex = 0;
    ...
    if (
      ...
      currentChild = parentNode._children[newChildIndex] = null;
      continue;
    }
    if (
    ...
    } else if (isArrayFn(currentChild)) {
      currentChild = parentNode._children[newChildIndex] = createVirtualNode(
        getChildrenFromNode,
        {
    ...
    const i = newChildIndex + indexOffset;
    currentChild._parent = parentNode;
    currentChild._depth = parentNode._depth + 1;
    const a = (currentChild._index = findMatchingSiblingIndex(
      currentChild,
      oldChildrenArray,
      i,
    ...
    matchedSibling = null;
    if (-1 !== a) {
      matchedSibling = oldChildrenArray[a];
      remainingOldChildrenCount--;
      if (matchedSibling) {
        matchedSibling._flags |= FLAG_MATCHED;
      }
    }
  ...
  if (remainingOldChildrenCount) {
    for (
      ...
      if (
        matchedSibling != null &&
        (matchedSibling._flags & FLAG_MATCHED) === 0
      ) {
        if (matchedSibling._dom == parentNode._nextDom) {
          parentNode._nextDom = findNextDomSibling(matchedSibling);
        }
...
function findMatchingSiblingIndex(
  ...
  let shouldSearchFurther =
    (typeof elementType != "function" ||
      elementType === getChildrenFromNode ||
      elementKey) &&
    searchDepth >
      (sibling != null && (sibling._flags & FLAG_MATCHED) === 0 ? 1 : 0);
  if (
    sibling === null ||
    (sibling &&
      elementKey == sibling.key &&
      elementType === sibling.type &&
      (sibling._flags & FLAG_MATCHED) === 0)
  ) {
    return currentIndex;
  }
...
function applyStyle(elementStyle, styleProperty, styleValue) {
  if (styleProperty[0] === "-") {
    ...
    NON_DIMENSIONAL_REGEX.test(styleProperty)
  ) {
  ...
  e: if (propertyName === "style") {
    ...
    if (newValue) {
      if (oldValue) {
        ...
        newValue._attached = eventTimestamp;
        element.addEventListener(
          propertyName,
          isCapture ? eventCaptureListener : eventListener,
          isCapture
        );
      }
    } else {
  ...
  return function (event) {
    if (this._listeners) {
      ...
      if (event._dispatched == null) {
        event._dispatched = eventTimestamp++;
...
var eventListener = createEventHandlerProxy(false);
var eventCaptureListener = createEventHandlerProxy(true);
function diffNodes(
  parentVNode,
  newVNode,
  oldVNode,
  ...
  if (oldVNode._flags & FLAG_SUSPENDED) {
    isHydrating = !!(oldVNode._flags & FLAG_HYDRATE);
  ...
  e: if (typeof componentType == "function") {
    try {
      ...
      diffChildrenNodes(
        parentVNode,
        isArrayFn(childrenToDiff) ? childrenToDiff : [childrenToDiff],
        newVNode,
        oldVNode,
        globalContext,
        namespaceURI,
      ...
      defaultOptions._catchError(loopIndex, newVNode, oldVNode);
    }
  } else if (
    childNodeArray == null &&
    newVNode._original === oldVNode._original
    ...
    newVNode._dom = patchElementTree(
      oldVNode._dom,
      newVNode,
      oldVNode,
      globalContext,
...
function commitRootFiberTree(rootArray, fiberNode, refArray) {
  fiberNode._nextDom = UNDEFINED_VALUE;
  for (let e = 0; e < refArray.length; e++) {
    setRef(refArray[e], refArray[++e], refArray[++e]);
  }
  ...
  rootArray.some((component) => {
    try {
      ...
      defaultOptions._catchError(renderError, component._vnode);
    }
  });
}
...
function setRef(ref, value, errorContext) {
  try {
    ...
    defaultOptions._catchError(error, errorContext);
  }
}
...
function createContextObject(defaultValue, contextName) {
  const context = {
    _id: (contextName = "__cC" + props++),
    _defaultValue: defaultValue,
    Consumer: (parent, selector) => parent.children(selector),
    Provider(element) {
      if (!this.getChildContext) {
        let e = new Set();
        let contextObject = {};
        contextObject[contextName] = this;
...
export {
  ...
  isValidElementInstance as isValidElement,
