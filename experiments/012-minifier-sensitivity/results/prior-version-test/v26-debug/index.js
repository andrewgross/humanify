var HYDRATE_MODE_FLAG = 32;
var SUSPENDED_MODE_FLAG = 128;
var INSERT_VNODE_FLAG = 4;
var MATCHED_FLAG = 2;
var CLEAR_MODE_MASK = ~(HYDRATE_MODE_FLAG | SUSPENDED_MODE_FLAG);
var SVG_NAMESPACE_URI = "http://www.w3.org/2000/svg";
var XHTML_NAMESPACE_URI = "http://www.w3.org/1999/xhtml";
var MATH_NAMESPACE_URI = "http://www.w3.org/1998/Math/MathML";
...
function renderComponentInstance(componentInstance) {
  let sourceVnode = componentInstance._vnode;
  let domNode = sourceVnode._dom;
  let fiberArray = [];
  let refArray = [];
  if (componentInstance._parentDom) {
    const mergedVnode = mergeObjects({}, sourceVnode);
    mergedVnode._original = sourceVnode._original + 1;
    ...
    diffNodes(
      ...
      sourceVnode._flags & HYDRATE_MODE_FLAG ? [domNode] : null,
      fiberArray,
      domNode == null ? findNextDomSibling(sourceVnode) : domNode,
      !!(sourceVnode._flags & HYDRATE_MODE_FLAG),
      refArray
    );
    mergedVnode._original = sourceVnode._original;
...
function processRerenderQueue() {
  let component;
  let previousQueueLength = 1;
  for (; pendingRerenderQueue.length; ) {
    if (pendingRerenderQueue.length > previousQueueLength) {
      pendingRerenderQueue.sort(sortByDepth);
    }
    component = pendingRerenderQueue.shift();
    previousQueueLength = pendingRerenderQueue.length;
    if (component._dirty) {
...
function diffChildrenNodes(
  ...
  for (
    ...
    if (
      ...
      nextDom !== UNDEFINED_VALUE
    ) {
      nextSiblingNode = nextDom;
    } else if (childDom) {
  ...
  if (remainingOldChildrenCount) {
    for (
      ...
      if (
        ...
        if (matchedSibling._dom == matchingIndex) {
          matchingIndex = findNextDomSibling(matchedSibling);
        }
...
processRerenderQueue._rerenderCount = 0;
var POINTER_CAPTURE_SUFFIX_REGEX = /(PointerCapture)$|Capture$/i;
var eventCounter = 0;
function setElementProperty(
  ...
  e: if (propertyName == "style") {
    ...
    isCapture =
      propertyName !=
      (propertyName = propertyName.replace(POINTER_CAPTURE_SUFFIX_REGEX, "$1"));
    if (
    ...
    if (namespace == SVG_NAMESPACE_URI) {
...
function diffNodes(
  ...
  if (oldVNode._flags & SUSPENDED_MODE_FLAG) {
    isHydrating = !!(oldVNode._flags & HYDRATE_MODE_FLAG);
  ...
  e: if (typeof componentType == "function") {
    try {
      ...
      newVNode._flags &= CLEAR_MODE_MASK;
      if (componentInstance._renderCallbacks.length) {
        componentCallbackQueue.push(componentInstance);
      }
      if (previousError) {
        ...
        if (loopIndex.then) {
          for (
            newVNode._flags |= isHydrating
              ? HYDRATE_MODE_FLAG | SUSPENDED_MODE_FLAG
              : SUSPENDED_MODE_FLAG;
  ...
  if (newVNode._flags & SUSPENDED_MODE_FLAG) {
    return undefined;
  } else {
    return domNode;
  }
...
function commitRootFiberTree(rootArray, fiberNode, refArray) {
  for (let renderError = 0; renderError < refArray.length; renderError++) {
    setRef(
      refArray[renderError],
      refArray[++renderError],
      refArray[++renderError]
    );
  }
  ...
  rootArray.some((component) => {
    try {
      ...
      defaultOptions._catchError(renderFailure, component._vnode);
    }
  });
}
function patchElementTree(
  ...
  if (newType == "svg") {
    ...
    namespace = MATH_NAMESPACE_URI;
  } else if (!namespace) {
    namespace = XHTML_NAMESPACE_URI;
  }
  ...
  if (newType === null) {
    ...
    if (newDangerouslySetInnerHTML) {
      ...
      diffChildrenNodes(
        ...
        oldVNode,
        parentVNode,
        newType == "foreignObject" ? XHTML_NAMESPACE_URI : namespace,
        existingChildNodes,
        childIndex,
        existingChildNodes
...
function setRef(ref, value, errorContext) {
  try {
    if (typeof ref == "function") {
      let hasUnmount = typeof ref._unmount == "function";
      if (hasUnmount) {
        ref._unmount();
      }
...
function unmountComponent(component, errorContext, skipDomRemoval) {
  ...
  if ((tempRefComponentChildren = component._component) != null) {
    if (tempRefComponentChildren.componentWillUnmount) {
      ...
      } catch (componentUnmountError) {
        defaultOptions._catchError(componentUnmountError, errorContext);
      }
    }
      ...
      this.sub = (component) => {
        ...
        component.componentWillUnmount = () => {
          if (subscribedComponents) {
            subscribedComponents.delete(component);
          }
          if (originalUnmount) {
...
export {
  ...
  isElementValid as isValidElement,
