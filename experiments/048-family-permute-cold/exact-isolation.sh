#!/usr/bin/env bash
#
# exp048 — the EXACT effect of the family-permute pass, isolated from LLM draws.
#
#   experiments/048-family-permute-cold/exact-isolation.sh [workdir]
#
...
run_leg() {
  ...
  echo "=== leg $LABEL (HUMANIFY_NO_FAMILY_PERMUTE='${HUMANIFY_NO_FAMILY_PERMUTE:-}') ==="
  rm -rf "$OUT"
...
HUMANIFY_NO_FAMILY_PERMUTE="" run_leg exp048-exact-on
MID=$(find "$CACHE" -type f 2>/dev/null | wc -l)
echo "cache entries after leg 1 (ON): $MID  (+$((MID - BEFORE)) written)"

HUMANIFY_NO_FAMILY_PERMUTE=1 run_leg exp048-exact-off
...
npx tsx -e "
const p='$RESULTS/';
const on=require(p+'exp048-exact-on/$TO.json').churn, off=require(p+'exp048-exact-off/$TO.json').churn;
...
"
echo
echo "moves shipped by the ON leg: $(grep -acE 'move [^ ]+ -> [^ ]+ \(support' "$RESULTS/exp048-exact-on/$TO.log" 2>/dev/null)"
