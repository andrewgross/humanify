...
use humanify_model::llm::{CacheKeyParams, cache_key_of};
use oxc_span::Span;

use crate::modules::vendor_names::{
    ManifestEntry, PriorManifestEntry, ProviderVendorNamer, VendorNameRequest, VendorNamer,
    accept_vendor_name, annotate_hash_ordinals, build_prompt, build_vendor_evidence,
...
// ---------------------------------------------------------------------------
// The vendor batch's cache key + replay — the LLM boundary's parity wiring
// ---------------------------------------------------------------------------
...
fn cache_key_matches_the_ts_vector() {
    let params = key_params(None);
    let requests = two_requests();
    assert_eq!(
        cache_key_of(&vendor_batch_request(&requests), &params),
...
fn cache_key_matches_the_oracle_recorded_vector() {
    let params = key_params(Some("low"));
    let batch: &[(&str, &str)] = &[
        (
            "lib_c0dcf0b5",
...
/// End to end through the real wiring: humanify-llm's replay-only client
/// (read-only disk cache) behind `ProviderVendorNamer`.
#[test]
fn replay_reads_the_sharded_entry_and_counts_misses() {
    ...
    let key = cache_key_of(&vendor_batch_request(&requests), &key_params(None));
...
/// The params every vendor test keys with: gpt-oss-20b, a literal
/// temperature 0 (unified.ts buildProvider), no max tokens.
fn key_params(reasoning_effort: Option<&str>) -> CacheKeyParams {
    CacheKeyParams {
...
fn two_requests() -> Vec<VendorNameRequest> {
    vec![
        VendorNameRequest {
            key: "lib_aaaabbbb".to_string(),
            evidence: "exports: parse\nsize: 10 bytes".to_string(),
