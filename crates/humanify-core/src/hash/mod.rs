...
pub mod partition_dump {
    ...
    /// must compare equal), and the statementHash family is rebuilt.
    pub fn dump_partitions(ts_dump_dir: &Path, out_dir: &Path) -> Result<usize, String> {
        ...
        let shipped =
            fs::read_to_string(&shipped_path).map_err(|e| format!("shipped text: {e}"))?;
        ...
        // Every wrapper-body statement, in bundle order, hashed with the
        // Rust statement hash. The ESTree JSON substrate per statement:
        // serialize the PROGRAM once (one JSON), then walk its wrapper
        // body statements — spans in the JSON carry byte offsets, so the
        // members key like the TS side's converted spans.
        let estree = ingest.program.to_estree_json(false, true);
        // The AST nests hundreds deep; serde_json's default recursion
        // limit (128) rejects it. Unbounded depth is safe here: the input
        // is oxc's own serialization of a program that parsed.
        let mut de = serde_json::Deserializer::from_str(&estree);
        de.disable_recursion_limit();
        let program_json = Value::deserialize(&mut de).map_err(|e| format!("estree json: {e}"))?;
        // The split's unit = the WRAPPER BODY's statements, not the
        // program's: descend the top-level expression statement into the
        // IIFE call/arrow's body (unwrapping oxc's parenthesized
        ...
        members.sort_by(|a, b| {
            let (as_, ae) = (
                a["member"]["start"].as_u64().unwrap_or(u64::MAX),
                a["member"]["end"].as_u64().unwrap_or(u64::MAX),
            );
            let (bs, be) = (
                b["member"]["start"].as_u64().unwrap_or(u64::MAX),
                b["member"]["end"].as_u64().unwrap_or(u64::MAX),
            );
            (as_, ae).cmp(&(bs, be))
        });
