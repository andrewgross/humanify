...
function renderLoop() {
  let component;
  let queueLength = 1;
  for (; componentRenderQueue.length;) {
    if (componentRenderQueue.length > queueLength) {
      componentRenderQueue.sort(sortByDepth);
    }
    component = componentRenderQueue.shift();
    queueLength = componentRenderQueue.length;
    if (component._dirty) {
...
function compareAndUpdateChildren(parentVNode, newChildren, parent, oldChildren, namespace, componentInstance, context, parentDom, nextSibling, refQueue, refUpdates) {
  ...
  for (nextSibling = buildChildrenArray(parent, newChildren, previousChildren, nextSibling, childrenCount), index = 0; index < childrenCount; index++) {
    ...
    let newChildrenVal = reconcileNodes(parentVNode, currentChild, prevChildData, namespace, componentInstance, context, parentDom, nextSibling, refQueue, refUpdates);
    ...
    } else if (typeof currentChild.type == "function" && newChildrenVal !== UNDEFINED_VALUE) {
      nextSibling = newChildrenVal;
    } else if (childDom) {
...
function buildChildrenArray(parentNode, newChildren, oldChildren, nextDom, unusedIndex) {
  let index, child, matchedChild;
  let matchedIndex = oldChildren.length;
  let remainingCount = matchedIndex;
  let offset = 0;
  for (parentNode._children = new Array(unusedIndex), index = 0; index < unusedIndex; index++) {
    child = newChildren[index];
    if (child == null || typeof child == "boolean" || typeof child == "function") {
      parentNode._children[index] = null;
      continue;
    }
  ...
  if (remainingCount) {
    for (index = 0; index < matchedIndex; index++) {
      ...
      if (matchedChild != null && (matchedChild._flags & MATCHED_FLAG) == 0) {
        if (matchedChild._dom == nextDom) {
          nextDom = findNextDomSibling(matchedChild);
        }
...
function locateMatchingElementIndex(currentNode, siblingArray, currentIndex, depthThreshold) {
  ...
  let shouldSearchNeighbors = depthThreshold > (siblingNode != null && (siblingNode._flags & MATCHED_FLAG) == 0 ? 1 : 0);
...
function setElementProperty(targetElement, attributeName, value, oldValue, namespace) {
  ...
  e: if (attributeName == "style") {
    ...
    isCapture = attributeName != (attributeName = attributeName.replace(CAPTURE_SUFFIX_REGEX, "$1"));
...
function createEventListenerProxy(eventKeySuffix) {
  return function (event) {
    if (this._listeners) {
      ...
      return listener(defaultOptionsVal.event ? defaultOptionsVal.event(event) : event);
    }
  };
}
...
function reconcileNodes(parentDom, vnode, componentInstance, globalContext, namespaceURI, domArray, callbackQueue, context, hydrateMode, renderCallbacks) {
  ...
  e: if (typeof componentIndex == "function") {
    try {
      ...
      context = compareAndUpdateChildren(parentDom, checkIfArray(renderedOutput) ? renderedOutput : [renderedOutput], vnode, componentInstance, globalContext, namespaceURI, domArray, callbackQueue, context, hydrateMode, renderCallbacks);
    ...
    } catch (stateCallbackIndex) {
      vnode._original = null;
      if (hydrateMode || domArray != null) {
        if (stateCallbackIndex.then) {
          ...
          for (let componentProcessingLoop = domArray.length; componentProcessingLoop--;) {
            removeDomNode(domArray[componentProcessingLoop]);
          }
        }
      } else {
        vnode._dom = componentInstance._dom;
      ...
      defaultOptionsVal._catchError(stateCallbackIndex, vnode, componentInstance);
    }
  } else if (domArray == null && vnode._original == componentInstance._original) {
    vnode._children = componentInstance._children;
    vnode._dom = componentInstance._dom;
  } else {
    context = vnode._dom = updateElementNode(componentInstance._dom, vnode, componentInstance, globalContext, namespaceURI, domArray, callbackQueue, hydrateMode, renderCallbacks);
  }
...
function updateElementNode(domElement, vnode, prevVnode, parent, namespace, childNodes, key, isHydrating, options) {
  ...
  if (elementType === null) {
    ...
    if (innerHtmlContent) {
      ...
      compareAndUpdateChildren(vnode.type === "template" ? domElement.content : domElement, checkIfArray(childrenArray) ? childrenArray : [childrenArray], vnode, prevVnode, parent, elementType == "foreignObject" ? XHTML_NS : namespace, childNodes, key, childNodes ? childNodes[0] : prevVnode._children && findNextDomSibling(prevVnode, 0), isHydrating, options);
      if (childNodes != null) {
...
function unmountComponent(componentInstance, parentContext, isRoot) {
  ...
  componentInstance._component = componentInstance._parent = componentInstance._dom = UNDEFINED_VALUE;
}
...
function createContextObject(defaultValue) {
  ...
  Provider._id = "__cC" + vnode++;
  Provider._defaultValue = defaultValue;
  Provider.Consumer = (parent, childSelector) => parent.children(childSelector);
  Provider.Provider = Provider._contextRef = Provider.Consumer.contextType = Provider;
  return Provider;
}
