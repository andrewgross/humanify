...
/**
 * exp076. A module that does NOT match mints a fresh path from
 * `inferFossilPlacements`, which re-derives the whole tree from the CURRENT
 * bundle's graph. Its matched neighbours meanwhile inherit their prior paths
 * verbatim, so the fresh module lands in a folder that the emitted tree may
 * not even contain — and re-derives differently next release, moving every
 * statement it holds. exp074 measured 567 statements changing file between
 * two fossil trees (old layout: 1), with folder churn on unmatched modules
 * the largest attributed class.
 *
 * The settled tree is the stable reference: place a fresh module where its
 * already-settled neighbours actually live.
 */
describe("assignFossil — fresh modules anchor to settled paths (exp076)", () => {
  // helper(module 0) is imported by three leaves(1,2,3) under one anchor(4).
  const src = bodyOf(
    [
      "var __esm = (fn, res) => () => (fn && (res = fn(fn = 0)), res);",
      "function helper(x) { return x; }",
  ...
  /** Prior ledger: every module keeps its real import edges, but the leaves
   * sit in a folder the inference would never mint, and the helper's content
   * is replaced so it cannot match and must mint fresh. */
  function priorWithLeavesAt(folder: string): StableSplitLedger {
    const first = assignFossil(src, hashes, undefined);
    const mods = first.fossilModules.map((m, i) => {
      if (i === 0) return { ...m, hashes: ["no-match-1", "no-match-2"] };
      return { ...m, file: `${folder}/mod-${i}.js` };
    });
    return {
      version: 1,
      files: [],
  ...
  it("an unmatched module lands in the folder its settled importers occupy", () => {
    const out = assignFossil(
      src,
      hashes,
      priorWithLeavesAt("src/renamed-zone")
    );
    // the leaves inherit verbatim...
    assert.strictEqual(out.assignment[3], "src/renamed-zone/mod-1.js");
    // ...and the fresh module joins them rather than minting an invented folder
    assert.strictEqual(folderOf(out.assignment[1]), "src/renamed-zone");
    assert.strictEqual(out.stats.signals.settledAnchor, 1);
  });

  it("follows the settled tree when it moves, instead of re-deriving", () => {
    const a = assignFossil(src, hashes, priorWithLeavesAt("src/zone-one"));
    const b = assignFossil(src, hashes, priorWithLeavesAt("src/zone-two"));
    assert.strictEqual(folderOf(a.assignment[1]), "src/zone-one");
    assert.strictEqual(folderOf(b.assignment[1]), "src/zone-two");
  });

  it("falls back to inference when no importer has a settled path", () => {
    // No prior at all: nothing is settled, so the existing signals decide
    // and the module still gets a home (never an empty or invented-root path).
    const out = assignFossil(src, hashes, undefined);
    assert.match(out.assignment[1], /^src\/.+\.js$/);
