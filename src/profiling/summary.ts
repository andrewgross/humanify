...
export function formatProfileSummary(report: ProfileReport): string {
  ...
  if (report.stageSummaries.length > 0) {
    ...
    const sorted = [...report.stageSummaries].sort(
      (a, b) => b.durationMs - a.durationMs
    );
    for (const stage of sorted) {
      const pct =
        total > 0 ? ((stage.durationMs / total) * 100).toFixed(1) : "0.0";
      lines.push(
        `  ${stage.name.padEnd(25)} ${formatDuration(stage.durationMs).padStart(10)}  (${pct}%)${stage.spanCount > 1 ? `  [${stage.spanCount} spans]` : ""}`
      );
    }
    lines.push("");
  }
  ...
  if (report.renameTiming) {
    ...
    lines.push(
      `  p50: ${formatDuration(rt.p50)}  p95: ${formatDuration(rt.p95)}  p99: ${formatDuration(rt.p99)}`
    );
    lines.push(
      `  min: ${formatDuration(rt.minMs)}  max: ${formatDuration(rt.maxMs)}`
    );
    lines.push("");
  }
  ...
  if (report.concurrencySnapshots.length > 0) {
    ...
    const avgInFlight =
      snapshots.reduce((s, c) => s + c.inFlight, 0) / snapshots.length;
    const maxInFlight = Math.max(...snapshots.map((c) => c.inFlight));
    const avgReady =
      snapshots.reduce((s, c) => s + c.ready, 0) / snapshots.length;
    lines.push("Concurrency utilization:");
    lines.push(
      `  avg in-flight: ${avgInFlight.toFixed(1)}  max in-flight: ${maxInFlight}`
    );
    lines.push(
      `  avg ready: ${avgReady.toFixed(1)}  samples: ${snapshots.length}`
    );
    lines.push("");
  }
