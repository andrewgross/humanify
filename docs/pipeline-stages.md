...
| --- | ------------------------ | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| 1   | Detect bundler/minifier  | `detectBundle` → `buildPipelineConfig`; directories go through `detectElectronApp` | **yes** — `--bundler` / `--minifier` override detection                              |
| 2   | Select unpack adapter    | `selectUnpackAdapter` (`src/unpack/index.ts`)                                      | **yes** — registry of 4, chosen by name, passthrough last                            |
...
standalone `split` command are all deleted; future input formats should join as
upfront detection + explicit pipeline pieces, not as fallbacks. **Electron did
exactly that (2026-08-23):** a directory input is detected upfront
(`detectElectronApp` — shape + markers, since a directory has no 16KB slice to
scan), routed to its own unpack adapter (`ElectronUnpackAdapter` — the app is
already a file tree, so unpack means selecting the humanify target under the
app's code root and leaving `node_modules` behind), and every flag the
multi-file input cannot honor (`--split`, `--prior-version`, the
last-file-only artifact flags) fails loudly upfront
(`checkDirectoryInputInvariants`) instead of silently half-working. The
upfront extraction (installer → app.asar → directory) lives OUTSIDE the
pipeline in `electron-unpack/`, exactly as Bun-executable JS extraction does.
