...
class on all four oracle pairs; the exact invocation is doc 07's. Dispatch
shape for every tier/adapter registry below is 02 §3's: closed enums with
exhaustive `match`, never trait objects; the LLM provider is a generic.
...
| id    | contents                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | TS-side prereq                | exit gate                                                                                                                 | depends on | parallel with | size      |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------- | --------- |
| WP1.1 | workspace scaffold (five crates per `02-rust-target-architecture.md` §2, incl. `humanify-model` so the parity differ builds first); env/kill-switch module: kill-switches.ts 191, env.ts, debug.ts 537, verbose.ts, cli.ts, index.ts, cli-error.ts, number-utils.ts, file-utils.ts, utils/ 57, shared/regex.ts, commands/default-args.ts 52 (cycle-breaker) → `humanify-cli` (the ONE env-reading module — core receives switches as config and never touches `std::env`) | none                          | `cargo build` + `cargo clippy` + `cargo test` green; kill-switch guard test ported (no `std::env` outside the env module) | WP0.4      | WPB.1         | M (989)   |
...
| id    | contents                                                                                                                                                                                                                                                                       | TS-side prereq                        | exit gate                                                                                                                                                                                                                                                                                                                                                                                     | depends on   | parallel with                           | size      |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ | --------------------------------------- | --------- |
| WP3.1 | validated rename + trail runner: rename/validated-rename.ts 568 (the seven rules), rename-ledger.ts 158, strategy-trail.ts 185, rename-eligibility.ts 60, five remaining small rename modules 227 → `core::rename::validated` + the generic trail-owning tier runner (`02` §5) | capture fixtures shared from TS suite | ported capture fixtures green; the `SymbolId → name` overlay is private to this module and `attempt_validated_rename` is its only writer; no module outside `core::emit` holds `&mut Scoping` (compile-checked via the shared-accessor wrapper, 02 §2) (amended 2026-09-19: was "symbol name field private to this module" — oxc's `Scoping` cannot be resealed; 02 §2's overlay design wins) | WP1.2        | WP2.x (build early, gate after phase 2) | L (1,198) |
...
| WP5.4 | scaffold/relink/vendor/carry: split/runnable-scaffold.ts 317, bun-relink.ts 277, vendor-body-inherit.ts 144, using-desugar.ts 96, post-split-reconcile.ts 502, bundle-carry.ts 344 → `core::finish`. Post-split reconcile is ported in its TEXT form for the 5a gate; the plan-based form (`12-layout-and-diff.md` §5) flips post-cutover behind its own warm byte-identity proof | none                               | **phase 5a gate: byte-identical emitted trees vs TS on warm replay** (`03-migration-plan.md` 5a); boot gate passes                                                                                                                                                                                                  | WP5.3, Track B | —                    | L (1,680)  |
...
| id    | contents                                                                                                                                                                                                                                                  | TS-side prereq                         | exit gate                                                                                                                                                           | depends on | parallel with | size       |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------------- | ---------- |
| WPB.1 | detection: detection/ 424 (detect.ts:35, signals/) → `core::detect` (pipeline stage, not a CLI concern — 02 §2)                                                                                                                                           | none                                   | detection verdicts equal TS on the fixture set                                                                                                                      | WP1.1      | WP2.x–4.x     | M (424)    |
| WPB.2 | unpack: unpack/ 1,424 (adapters/bun.ts 827 native; webcrack via subprocess shim absorbing plugins/webcrack.ts 127; vendor-namer.ts, manifest-order.ts) → `core::unpack` (02 §2)                                                                           | unpack manifest + vendor table in dump | differ: unpack manifest + vendor-name carry identical (fresh vendor names gate at warm replay)                                                                      | WPB.1      | WP2.x–4.x     | L (1,551)  |
| WPB.3 | library detection: library-detection/ 547 (index.ts:14 registry, comment-regions, banner-patterns, adapters/) → `core::libdetect` (02 §2)                                                                                                                 | none                                   | detector selections + regions equal TS on the fixture set                                                                                                           | WPB.2      | WP2.x–4.x     | M (547)    |
