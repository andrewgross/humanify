...
| (Rust) A JS regex's character classes and a JS string index                | `crate::detect::js_text` — `is_js_space` (`\s`), `is_word_boundary` (ASCII `\b`), `js_prefix` (`slice(0, n)` in UTF-16 units), `utf16_offset` (a byte offset as the JS index)    | every hand-emulated V8 regex in `detect`, `unpack`, `libdetect` builds on these; never Rust's `char::is_whitespace` (U+FEFF / U+0085 disagree — the WPB.3 probe's planted red)                                                                                                                                                                    |
