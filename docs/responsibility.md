...
| Where is an extracted Electron app's code?             | `resolveElectronAppLayout` — `src/unpack/electron-layout.ts`                                                                                                                     | package.json `main` with node-style resolution (Electron's own boot rule); code root = main's top-level dir (`out`/`dist`/`.`). Detection AND the electron unpack adapter both call it — the answer must not fork                                                                                                                                 |
| Is this directory an extracted Electron app?           | `detectElectronApp` — `src/detection/electron-app.ts`                                                                                                                            | layout resolvable → yes; electron markers (dep names, entry referencing `"electron"`) decide definitive vs likely. Minifier still comes from the entry's CODE signals — packaging routes the adapter, the toolchain routes skip-lists                                                                                                             |
