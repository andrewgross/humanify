...
| Which apply site actually runs a function rename?      | the WAVE BARRIER (`processor.ts` ~:2335, via the `WaveNodeCtx` applier) under the default scheduler; the `applyRename` callback in `makeCallbacks` (~:486) ONLY with `--no-wave-scheduling` | two call sites, one live. Instrumenting the obvious one measures nothing: a probe there recorded `applies=0` across a run of ~175,000 renames. Wave scheduling collects renames and applies them at the barrier in deterministic order (`wave-scheduler.ts:9`)                                              |
